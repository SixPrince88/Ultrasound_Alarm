 ...
 * 返 回 值：无
 */
void HW_OLED_I2C_Init(void)
{
	/*将SCL和SDA引脚初始化为开漏模式*/
	gpio_init_t oled_pins = {.gpio_x = GPIOA,.pin_x = GPIO_Pin_12 | GPIO_Pin_15,.mode_x = MODE_OOD,};
	User_GPIO_All_Init(&oled_pins, 1);
	//	/*释放SCL和SDA*/
	OLED_W_SCL(1);
	OLED_W_SDA(1);
}
...
{
	gpio_init_t oled_pins[2] = {
		{.gpio_x = GPIOB,.pin_x = GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15,.mode_x = MODE_OPP,},
		{.gpio_x = GPIOA,.pin_x = GPIO_Pin_8,.mode_x = MODE_OPP,}
	};
	User_GPIO_All_Init(oled_pins, 2);
 ...
 * 返 回 值：无
 */
void OLED_I2C_WriteCommand(uint8_t Command)
{
 ...
 * 返 回 值：无
 */
void OLED_I2C_WriteData(uint8_t *Data, uint8_t Count)
{
	uint8_t i;
 ...
 * 返 回 值：无
 */
void OLED_SPI_WriteData(uint8_t *Data, uint8_t Count)
{
	uint8_t i;
...
{
	OLED_ShowNum(1, 4, fd07_3_dist[0],OLED_8X16, 4);
	OLED_ShowNum(1, 12, fd07_3_dist[1],OLED_8X16, 4);
	OLED_ShowNum(2, 4, fd07_3_dist[2],OLED_8X16, 4);
	OLED_ShowNum(2, 12, fd07_3_dist[3],OLED_8X16, 4);
	OLED_ShowNum(3, 4, fd07_3_dist[4],OLED_8X16, 4);
	OLED_ShowNum(3, 12, fd07_3_dist[5],OLED_8X16, 4);
}
/*****************江协科技|版权所有****************/
/*****************jiangxiekeji.com*****************/
